...
const props = defineProps<{
  invocation: AreaChartUIToolInvocation;
}>();
...
const icon = computed(() => {
  return ({
    'input-available': 'i-lucide-area-chart',
...
const xFormatter = (invocation: AreaChartUIToolInvocation) => {
...
const categories = (invocation: AreaChartUIToolInvocation): Record<string, BulletLegendItemInterface> => {
  ...
  return invocation.output.series.reduce((acc: Record<string, BulletLegendItemInterface>, serie: { key: string; name: string; color: string }) => {
    acc[serie.key] = { name: serie.name, color: serie.color };
    return acc;
...
<template>
  <div v-if="invocation.state === 'output-available'" class="my-5">
    <div v-if="invocation.output.title" class="flex items-center gap-2 mb-2">
      <UIcon name="i-lucide-area-chart" class="size-5 text-primary shrink-0" />
      <div class="min-w-0">
            ...
            <div class="space-y-1.5">
              ...
              >
                <div class="flex items-center gap-2 min-w-0">
                  <div class="size-2.5 rounded-full shrink-0" :style="{ backgroundColor: serie.color }" />
